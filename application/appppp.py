...
class FileUploader(QWidget):
    ...
    def download_pdf(self):
        ...
        background_image_path = "application/light blue.jpg"  # Background image path
        ...
        cell_width = 45  # Width of each cell for a consistent grid
        row_height = 10  # Row height for each cell

        # Define headers and values for Scan Information
        headers = ["Examination", "Indication", "Tumor", "Tumorous", "Overview", "Severity", "Dimensions", "Therapies", "Removable"]
        values = ["MRI Brain", "Suspected Brain Tumor", "", "", "", "", "", "", ""]

        # Draw headers row
        for header in headers:
            pdf.cell(cell_width, row_height, header, border=1, align='C')
        pdf.ln(row_height)

        # Draw values row
        pdf.set_font("Arial", '', 12)
        for value in values:
            pdf.cell(cell_width, row_height, value, border=1, align='C')
        pdf.ln(row_height)

        # Add uploaded image if not DICOM
        pdf.set_font("Arial", size=12)
        if not self.file_label.text().lower().endswith('.dcm'):
            image_path = self.file_label.text().replace("Selected File: ", "")
            if os.path.exists(image_path):
                pdf.ln(10)
                pdf.image(image_path, x=15, y=None, w=180, h=120)
