...
class FileUploader(QWidget):
    ...
    def download_pdf(self):
        ...
        # Add a background image for the PDF (use an embedded image here)
        background_image_path = "application/A4.jpeg"  # Place your image in the project folder and set the correct path
        if os.path.exists(background_image_path):
            # Draw the background image on the entire page (A4 size: 210x297 mm)
            pdf.image(background_image_path, x=0, y=0, w=210, h=297)

        # Set font for the title
        pdf.set_font("Arial", 'B', 16)
        pdf.set_text_color(0, 0, 0)  # Set the text color (adjust this if the background makes it hard to read)
